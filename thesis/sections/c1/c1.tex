...
The columns have the following meaning: Model containing the names of each one of the available LLMs (for the purpose of this paper, only text generation models were selected), moving to the right \textit{RPM} contains the maximum number of requests in a minute, \textit{RPD} contains the maximum number of requests per day, \textit{TPM} contains the maximum number of requested tokens per minute, and finally \textit{TPD} contains the maximum number of tokens per day.
...
\begin{itemize}
    \item Minimize prompt-induced bias: An excessively descriptive or overly complex prompt could negatively influence model behavior, since long prompts may lead to phenomena such as ``context rot''~\cite{ContextRot} - a progressive decay in accuracy as prompts grow longer.
...
Each problem is a combination of a \texttt{.mzn} file containing the MiniZinc~\cite{MiniZinc} model, containing variables, constraints and objective function. Every problem is also accompanied by five corresponding data instances, each of them contained either in a \texttt{.dzn} or a \texttt{.json} file containing specific parameters and constants, yielding a total of 100 testable, diverse, and complex scenarios.

\section{Test Metrics}
\label{sec:metrics}
In order to actually evaluate LLM performance, it is necessary to choose a standard metric for answer evaluation. In addition, it is necessary to have a metric to evaluate how each given LLM would perform against the current Single Best Solver (SBS).

Before analyzing the evaluation metrics, we must first define the systems to which these metrics will be applied. Namely, the solvers. In our context, a solver is a program that takes as input the description of a computational problem in MiniZinc, with relative data, and returns an observable outcome consisting of zero or more solutions for the given problem.  
...
The parameters used for score calculation in testing are: $T=1200000$ (1200000ms = 20 minutes, which is the time limit used for solver evaluation in the MiniZinc Challenge) $\alpha=0.25$ $\beta=0.75$.
...
For this objective, we have chosen to use closed-gap (CG)~\cite{evaluationMetaSolvers} as the evaluation metric. 
CG is a relative and meta-solver-specific measure, adopted in the 2015 ICON and 2017 OASC ~\cite{lindauer2019}
...
\label{sec:expSetup}
We have defined both the structure of the queries posed to the LLMs (\Cref{sec:probSelection}) and the way in which these queries are formulated (\Cref{sec:promptStruct}). The next step is to evaluate them automatically. over the full set of selected instances.
...
A first step consisted of removing all non-essential elements, such as comments (starting with \texttt{\%} ~\cite{MiniZinc}), tabs, and unnecessary whitespace. While this helps reduce token usage and standardizes script formatting, it is insufficient on its own. The main contributor to token overflow is the presence of large data arrays, which not only increase message length but may also introduce irrelevant information into the context, ``distracting'' the LLM from the most relevant information~\cite{LLMdistract}.
