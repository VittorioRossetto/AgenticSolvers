...
# This script was copied from the github repository: https://github.com/informarte/minizinc-challenge-results
...
def importResults(file, cursor):
    ...
    # Handle both formats: old flat JSON with "year" and new nested JSON
    if "results" in raw:
        year = raw.get("year", 2025)  # fallback if not present
        results = raw["results"]
    else:
        year = raw["year"]
        results = raw
    ...
    cursor.executemany(
        ...
        zip(solvers,
            results['fd_solvers'],
            results['free_solvers'],
            results['par_solvers'],
            results['open_solvers'])
    )
    cursor.executemany('INSERT INTO problem VALUES(?, ?)',
                       zip(problems, results['kind']))

    jobs = [
        (problem, instance)
        for (problem, instances) in zip(
            problems,
            map(lambda refs: map(lambda ref: benchmarks[ref], refs),
                results['instances'])
        )
        for instance in instances
    ]

    for (solver, states, times, objectives) in zip(
        solvers,
        results['results'],
        results['times'],
        results['objectives']
